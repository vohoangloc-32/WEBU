  ...
  return (
    <div className="bg-[#111827] border border-white/10 rounded-xl px-4 py-3 shadow-xl text-xs">
      ...
      <div className="space-y-1 text-neutral-400">
        <div>
          Skill Score:{' '}
          <span className="text-emerald-400 font-bold">{d.skill_score}</span>
          /100
        </div>
        <div>
          Pass Rate: <span className="text-blue-400">{d.pass_rate}%</span>
        </div>
        <div>
          Passed:{' '}
    ...
    <div className="p-5 rounded-2xl bg-white/[0.04] border border-white/[0.08]">
      ...
      <div className="mt-2 grid grid-cols-2 gap-1">
        {data.slice(0, 6).map((d) => (
          <div
            key={d.tag}
            className="flex items-center justify-between px-2 py-1 rounded-lg bg-white/[0.03]"
          >
